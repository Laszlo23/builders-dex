...
interface ProjectSubmitFormProps {
  ...
  onConnect: () => void;
  onSubmit: (project: SubmitPayload) => void | Promise<void>;
  compact?: boolean;
  ...
  return (
    <form onSubmit={handleSubmit} className={`space-y-5 ${compact ? '' : ''}`}>
      ...
      <button
        ...
        className="w-full rounded-xl bg-accent py-3.5 text-sm font-bold text-ink hover:bg-accent-bright disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
      >
        {isSubmitting ? (
          <>
